...
function parseArgs() {
  ...
  const options = {
    lang: "en",
    totalSeconds: Number(process.env.TOUR_TOTAL_SECONDS || 60),
    slidesDir: null,
    outVideo: null,
    outAudio: null,
  };
  ...
  for (let i = 0; i < args.length; i += 1) {
    switch (args[i]) {
      ...
      case "--out-audio":
        ...
        break;
      case "--total":
        options.totalSeconds = Number(args[++i] || 60);
        break;
      default:
        break;
    }
...
async function buildVideoFromSlides({ slides, audioPath, outputPath, seconds }) {
  const perSlide = Math.max(3, seconds / slides.length);
  const listPath = join(tmpdir(), `islapos_slides_${Date.now()}.txt`);
...
async function main() {
  ...
  const slidesDir = options.slidesDir || join(repoRoot, "public", "islapos-tour", lang);
  const outVideo = options.outVideo || join(repoRoot, "public", "videos", `islapos-${lang}.mp4`);
  const outAudio = options.outAudio || join(repoRoot, "public", "videos", `islapos-${lang}.mp3`);

  const scriptPath = join(repoRoot, "scripts", "video", `script_tour_${lang}.txt`);
