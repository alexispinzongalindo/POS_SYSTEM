...
export default function SeeAppInActionPage() {
  ...
  const slidesUrl = useMemo(() => `/islapos-tour/${langKey}/slides.json`, [langKey]);
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-[1.35fr_0.65fr]">
                ...
                <div className="flex h-full w-full items-center justify-center px-8 text-center text-sm text-[var(--mp-muted)]">
                  {!slidesLoaded
                    ? langKey === "es"
                      ...
                      : "Loading tour…"
                    : langKey === "es"
                      ? "No hay slides todavía. Genera el tour con: npm run capture:tour:slides:es"
                      : "No slides yet. Generate the tour with: npm run capture:tour:slides:en"}
                </div>
              )}
            </div>
          ...
          <div className="rounded-3xl border border-[var(--mp-border)] bg-white p-6 shadow-sm">
            <div className="text-lg font-semibold">
              {slide ? slide.title[langKey] : langKey === "es" ? "Tour" : "Tour"}
            </div>
            <div className="mt-2 whitespace-pre-line text-sm text-[var(--mp-muted)]">
              {slide
                ? slide.body[langKey]
                : langKey === "es"
                  ? "Genera los slides del tour desde tu app real. Esto también evita pantallas negras o de error porque se capturan solo cuando la UI está lista."
                  : "Generate the tour slides from your real app. This also prevents black/error screens because we only capture after the UI is ready."}
            </div>
